...
class DataCollector:
	...
	def __init__(self):
		...
		self.authors = {} # name -> {commits, first_commit_stamp, last_commit_stamp, last_active_day, active_days, lines_added, lines_removed, active_days_in_last_60_days, active_days_in_last_90_days}
		...
		self.active_days_in_last_60_days = set()
		self.active_days_in_last_90_days = set()
...
class GitDataCollector(DataCollector):
	def collect(self, dir):
		...
		for line in lines:
			...
			if 'last_active_day' not in self.authors[author]:
				...
				self.authors[author]['active_days_in_last_60_days'] = set([])
				self.authors[author]['active_days_in_last_90_days'] = set([])
			elif yymmdd != self.authors[author]['last_active_day']:
				self.authors[author]['last_active_day'] = yymmdd
				self.authors[author]['active_days'].add(yymmdd)
				if (datetime.datetime.now() - date).days <= 60 :
					self.authors[author]['active_days_in_last_60_days'].add(yymmdd)
				if (datetime.datetime.now() - date).days <= 90 :
					self.authors[author]['active_days_in_last_90_days'].add(yymmdd)
		...
		for author in self.authors:
			self.authors[author]['active_days_in_last_60_days'] = list(self.authors[author]['active_days_in_last_60_days'])
			self.authors[author]['active_days_in_last_90_days'] = list(self.authors[author]['active_days_in_last_90_days'])
			self.authors[author]['active_days'] = list(self.authors[author]['active_days'])
	...
	def getAuthors(self, limit = None):
		res = getkeyssortedbyvaluekey(self.authors, 'last_commit_stamp')
		res.reverse()
		return res[:limit]
